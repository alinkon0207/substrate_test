...
impl Runtime {
	...
	fn do_finalize_block() -> <Block as BlockT>::Header {
		...
		// set correct state root and extrinsics root
		header.state_root = BlakeTwo256::hash_of(&sp_io::storage::root(VERSION.state_version()));
		header.extrinsics_root = BlakeTwo256::hash_of(&sp_io::storage::get(&EXTRINSICS_KEY));
	...
	fn do_apply_extrinsic(ext: <Block as BlockT>::Extrinsic) -> ApplyExtrinsicResult {
		...
		match ext.call.function {
			RuntimeCall::System(shared::SystemCall::Set { value }) => {
				Self::mutate_state(&VALUE_KEY, |val: &mut u32| *val = value);
			}
			_ => return Err(InvalidTransaction::Call.into()),
		};

		// Record the extrinsic by encoding it and pushing it into the list of extrinsics
		Self::mutate_state(&EXTRINSICS_KEY, |extrinsics: &mut Vec<Vec<u8>>| {
	...
	fn do_validate_transaction(
		...
		/* match ext.function {
			RuntimeCall::System(shared::SystemCall::Set { value: _ }) => {
				// Check if the transaction signer is known
				if let Some((who, _, _)) = ext.signature.clone() {
					let lookup: <Block as frame_system::Config>::Lookup = RuntimeCall:lookup();
					if lookup.lookup(who.clone()).is_ok() {
						// Transaction is valid
						return Ok(ValidTransaction {
							requires: vec![],
							provides: vec![who.0.into()],
							priority: 0,
		...
		// If we get here, the transaction is invalid
		Err(TransactionValidityError::Invalid(InvalidTransaction::Custom(1u8))) */

		let signature_payload = ext.signature.clone();
		let function_call = ext.function.clone();
	
		// Perform signature verification if the signature is present
		if let Some(signature_payload) = signature_payload {
			let public_key = signature_payload.0;
			let signature = signature_payload.1;
			let encoded_call = function_call.encode();
			...
			if !valid_sig {
				// If the signature is not valid, return an invalid transaction
				return Err(TransactionValidityError::Invalid(InvalidTransaction::BadProof)); // You can use any custom error code here.
			}


		}
	
		// If the signature is valid or there is no signature, return a valid transaction
		Ok(ValidTransaction {
			provides: vec![ext.encode()],
			..Default::default()
		})
